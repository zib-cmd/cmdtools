...
def q_doublewell(dim,beta):
    ...
    # potential
    u = np.exp(-(grid**2-2)**2)
    G = nx.erdos_renyi_graph(dim+1,0.2)
...
def obtain_q(dim=50, density=0.05):
#    infgen = random(dim, dim, density).toarray()
    infgen = q_doublewell(dim-1, beta = 10)
#    print(np.shape(infgen))
#  infgen = sqra.sqra()
    infgen[np.diag_indices(dim)] = - np.sum(infgen- np.diagflat(np.diagonal(infgen)), axis=1)
    return(infgen)
def obtain_k(tau_max, step, dim=50, density=0.05, noise=False):
    infgen = obtain_q(dim, density)
...
k , q = obtain_k(6, 1, dim=10, density=.7, noise =False) 

alpha = compare_eigenspace(k, q)

#%%
dw = q_doublewell(50,10)
